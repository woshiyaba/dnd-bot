...
- PC 端支持探索入口、在场人物交谈、已访问地点、先攻顺序、随身背包和队友间物品转交。
- 玩家可在「创作并发布剧本」中提交自己的构思或大纲，经真实 LLM 生成、校验后发布；广场支持搜索与分享链接。

## PC 跑团与剧本分享

开发时先启动后端，再运行前端 `npm run dev`；Vite 会把 `/api` 和 `/ws` 转发到本机的 32388 端口。
需要让同伴访问时，在 `front/pc-dnd-bot` 运行 `npm run build`，然后启动或重启 `uv run python main.py`。
后端会同时提供网页、API 和 WebSocket。用同伴能访问的服务器地址打开网页，复制广场中的剧本链接即可分享。
`localhost` 只指向访问者自己的电脑；跨设备请使用服务器的局域网地址或已配置的域名。
前后端分开部署时仍可用 `VITE_API_BASE_URL` 指定后端地址。

玩法循环为：选择或创作剧本 → 创建角色与房间 → 自由探索/交谈 → 必要时检定 → 回合战斗 → 战后继续探索或结局。
探索按钮提交的是行动意图，移动与发现由 DM 结合剧本判定。背包操作与战斗遵循以下规则：

- 新角色携带两瓶治疗药水，可在探索中使用，或在战斗回合内消耗一个行动；治疗为 `2d4+2`，不能超过最大生命。
- 本版仅区分正常和倒下，倒下角色接受治疗后可恢复行动；尚未实现死亡豁免与永久死亡。
- 战斗道具的目标受到距离约束，探索时同队角色共同处于当前地点。物品扣除跨掷骰中断只执行一次。
- 背包转交仅允许在探索空闲时进行，不能在检定、战斗或待升级时绕过行动流程；服务器验证库存、数量及队友身份。
- 已发现线索和实际背包持续跟随本局角色，战斗中的数量、技能充能和先攻按当前战斗状态展示。

这是基于现有规则的轻量桌面跑团实现，尚未完整覆盖 D&D 5e 的动作/附赠动作/反应、休息和法术位。
已发布剧本保存为 `canon/*.json`，生成任务保存于 SQLite；房间和游戏检查点仍在内存中，后端重启会丢失正在进行的游戏。
微信小程序继续兼容原协议，本次新增背包和探索面板优先提供给 PC 端。
