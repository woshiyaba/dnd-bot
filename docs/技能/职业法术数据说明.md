...
`base`、`method` 和 `on_save` 适合程序读取；`scaling` 与 `details` 仍是中文规则摘要。它们用于后续理解和规则实现，不应直接交给 LLM 临场计算，也不应让引擎通过解析自然语言来扣 HP。
...
`dnd_skill/` 目前是静态资料，还没有直接接入战斗引擎：

- `src/model/effects.py` 中的 `LearnedSkill` 只保存 `skill_id`、`charges` 和 `cooldown_left`；
- `LearnedSkill.is_available` 当前同时要求 `charges > 0` 且 `cooldown_left <= 0`；
- `src/combat/nodes.py` 当前只对少量已知技能做硬编码结算，并没有通用法术效果解释器；
- 当前示例技能 ID 使用 `skill_second_wind`，而法术目录使用 `fireball` 这类法术 ID，尚未建立统一注册表。

因此，后续接入时不能只把法术 ID 填入角色卡就认为已经可执行。至少还要明确：

1. 法术 ID 与技能注册表的统一命名规则；
2. 冷却制下 `charges` 如何赋值，或是否调整可用性判断；
3. `cooldown_left` 的设置和递减时机；
4. 法术攻击、豁免、伤害、治疗、状态和专注的效果积木；
5. 玩家骰中断与怪物引擎掷骰的交互；
6. 专注被打断及同时只能维持一个专注法术的规则。

仍应遵循项目原则：**规则归引擎，叙述归 DM，骰子归玩家**。命中、豁免、伤害、治疗、HP、冷却和专注都应由确定性代码结算；LLM 只负责裁定与叙述。
