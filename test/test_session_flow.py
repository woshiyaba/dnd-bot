...
def build_scene() -> dict:
    ...
    return {
        ...
        "scene": {
            ...
            "actors": [
                {
                    "actor_id": "goblin_1", "name": "哥布林·斥候",
                    "disposition": "hostile", "type": "monster",
                    "card": {
        ...
        "party": [
            {
                "type": "player", "controller": "user_aria",
                "card": {
                    "id": "pc_aria", "name": "艾莉亚",
...
def auto_respond(interrupt: dict) -> dict:
    ...
    print(f"    >> 中断[{kind}] 面向 {who.get('combatant_id')}/{who.get('user_id')}：{interrupt.get('prompt')}")
...
def _decide_action(options: dict) -> dict:
    ...
    for weapon in options.get("attack", []):
        ...
        if targets:
            ...
            return {"action_type": "attack", "attack_name": weapon["attack_name"],
...
async def _drive_to_rest(engine: SessionEngine, room_id: str, payload: dict, max_steps: int = 200) -> dict:
...
async def run_flow() -> dict:
    ...
    print("\n" + "=" * 64); print("② 玩家做一件不确定的事 → 明检定（中断）"); print("=" * 64)
    payload = await engine.message(room, "我仔细搜索这些碎石和石柱，看看有没有线索。")
    assert payload["status"] == "interrupted", f"应触发明检定中断，实际={payload['status']}"
    ...
    print("\n" + "=" * 64); print("③ 玩家开战 → 进战斗子图 → 逐骰打完"); print("=" * 64)
    payload = await engine.message(room, "我拔剑冲上去攻击那只哥布林！")
    assert payload["status"] == "interrupted", f"应进入战斗并停在第一个骰，实际={payload['status']}"
    payload = await _drive_to_rest(engine, room, payload)
...
def test_session_flow() -> None:
    ...
    assert payload["status"] == "awaiting_input"
    # 战斗确实发生且玩家胜
    assert (state.get("last_combat") or {}).get("outcome") == "players_win", \
        f"应玩家获胜，实际={state.get('last_combat')}"
    # 被击败的哥布林已从场景移除
    assert all(a.get("actor_id") != "goblin_1" for a in (state["scene"].get("actors", []))), \
        "战败的哥布林应已从场景中移除"
    # 事件流非空，且含一条 combat 事件
    assert any(e.get("event") == "combat" for e in state.get("campaign_log", [])), "缺少 combat 事件"
